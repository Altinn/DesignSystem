var pkg = require('./package.json'); var gulp = require('gulp')
var gulp_concat = require('gulp-concat')
var gulp_rename = require('gulp-rename'); var path = require('path')
var fs = require('fs'); var eol = require('os').EOL; var del = require('del')
var strip_banner = require('gulp-strip-banner')
var header = require('gulp-header'); var nodeunit = require('gulp-nodeunit')
var eslint = require('gulp-eslint'); var sass = require('gulp-sass')
var browserSync = require('browser-sync').create()
var pjson = require('./package.json'); var version = pjson.version
...
gulp.task('banner', function () {
  return gulp.src([
    './core/lib/patternlab.js', './core/lib/object_factory.js',
    './core/lib/lineage_hunter.js', './core/lib/media_hunter.js',
    './core/lib/patternlab_grunt.js', './core/lib/patternlab_gulp.js',
    './core/lib/parameter_hunter.js', './core/lib/pattern_exporter.js',
    './core/lib/pattern_assembler.js', './core/lib/pseudopattern_hunter.js',
    './core/lib/list_item_hunter.js', './core/lib/style_modifier_hunter.js'
  ])
  .pipe(strip_banner())
  .pipe(header(banner, { pkg: pkg, today: new Date().getFullYear() }))
  .pipe(gulp.dest('./core/lib')) })
// Clone core folder, add Gulp-file and modify some JS:
...
gulp.task('cp:dcss', function () {
  fs.readFile('./source/css/style.scss', 'utf-8',
    function (err, custom) {
      if (err) console.log(err)
      var src = custom.replace(
        '@import "scss/base/profile-presentation"; ',
        '// Automatically removed')
      src = src.replace(
        '@import "scss/base/pattern-presentation"; ',
        '// Automatically removed')
...
gulp.task('cp:djs', function () {
  return gulp.src([
    ...
    'source/js/altinn.js']).pipe(gulp_concat('concat.js'))
...
gulp.task('cp:sge', function () {
  return gulp.src(path.resolve(paths().source.css, 'styleguide-edit.scss'))
    .pipe(sass().on('error', sass.logError))
    .pipe(gulp.dest('public/styleguide/css')).pipe(browserSync.stream())
})
// Create patternlab-presentation.css:
gulp.task('cp:pp', function () {
  return gulp.src(
    path.resolve(paths().source.css, 'patternlab-presentation.scss'))
    .pipe(sass().on('error', sass.logError)).pipe(gulp.dest('public/css'))
    .pipe(browserSync.stream())
})
// Copy styleguide:
...
gulp.task('connect', ['lab'], function () {
  ...
  gulp.watch(
    ...
    ['lab-pipe'], function () { browserSync.reload() }
  )
})
// Lint
...
gulp.task('assets', [
  ...
  'cp:font', 'cp:data', 'cp:css', 'cp:djs', 'cp:dcss', 'cp:sge', 'cp:pp',
  'cp:styleguide'
])
    ...
    '<script>' +
      '$("#sg-viewport").on("load", function () {' +
        '$("#sg-switchtheme-blue",' +
          ...
          '$("body", $("iframe").contents()[0])' +
            '.toggleClass("private-person");' +
